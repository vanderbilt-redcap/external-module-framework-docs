...
redcap_module_todo_list_item_add($request_id, $request_from_user, $request_to_email, $todo_type, $action_url, $project_id, $comment, $todo_type_id) | TBD | Triggered when a new item is added to the Control Center's ToDo List. Note that `$request_from` is the `username` of the user who initiated the request, and `$request_to_email` is the email address of the corresponding admin.
