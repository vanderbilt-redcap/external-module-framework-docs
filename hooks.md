...
redcap_module_api_before($project_id, $content, $action) | TBD | Triggered just before REDCap's API is called (i.e., after REDCap has determined which API method is going to be called but before actually calling it).  `$content` is the content of the API call, and `$action` is the name of the action being taken by the API call. For example, if the REDCap API call is `Export Records`, then `$content` will be _"record"_ and  `$action` will be _"export"_. _Note: `$action` will not necessarily match the value of `action` in the body of the user's API request. The user's actual request details can be obtained by accessing the $\_POST superglobal within the hook function._
