...
redcap_module_todo_list_item_add($request_id, $request_from, $request_to, $todo_type, $action_url, $project_id, $comment, $todo_type_id) | TBD | Triggered when a new item is added to the Control Center's ToDo List. Note that `$request_from` and `$request_to` are the `ui_id` for the corresponding user, _not_ their `username`.
