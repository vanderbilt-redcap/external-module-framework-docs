...
getChoiceLabels($fieldName[, $pid]) | 8.0.0 | Returns an associative array mapping all choice values to labels for the specified field.
...
Method<br><br>&nbsp; | Minimum<br>REDCap<br>Version | Description<br><br>&nbsp;
--- | --- | ---
~~getChoiceLabel($fieldName, $value, [, $pid])~~ | 8.0.0 | Deprecated in favor of `getChoiceLabels($fieldName)[$value])`, and due to inconsistent behavior and performance concerns from the unnecessary `getData()` call.  Get the label associated with the specified field & value.  The project ID parameter will be automatically detected if possible.
