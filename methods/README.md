...
`getUserQuery`($key [, $pid]) | 17.0.7 | Returns the user query with the specified key that was saved with the `saveUserQuery()` method. If a project ID is provided, the query for that project is returned. The resulting query can be executed with the `runUserQuery()` method. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
`getUserQueryKeys`([$pid]) | 17.0.7 | Returns an array of user query keys for the specified project ID. If no project ID is provided, system query keys are returned. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
...
`runAdminQuery`($sql [, $parameters]) | 17.0.7 | Executes the provided query against the database using the provided optional parameters. Only `select` queries are allowed. This method can be used with any `select` query, but only administrators with the **Access to all projects and data with maximum user privileges** *AND* the **Install, upgrade, and configure External Modules** permissions.
`runUserQuery`($sql [, $parameters]) | 17.0.7 | Executes the provided query against the database using the provided optional parameters. Only `select` queries are allowed. This method can only be used with queries returned by the `getUserQuery()` method. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
...
`saveUserQuery`($key, $sql [, $pid]) | 17.0.7 | Saves a user query with the specified key and SQL statement. If a project ID is provided, the query is saved for that project only. The query can be retrieved by the `getUserQuery()` method and then executed by the `runUserQuery()` method. Requires **Access to all projects and data with maximum user privileges** permission. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
