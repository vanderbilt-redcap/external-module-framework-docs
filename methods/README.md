...
`apiResponse`($body = ""), `apiErrorResponse`($error_message = "", $status = 500), `apiFileResponse`($path, $filename = "", $type = "text/plain"), `apiFileContentsResponse`($contents, $filename = "", $type = "text/plain"), `apiJsonResponse`($data, $force_object = false, $flags = 0), `apiJsonFileResponse`($data, $filename, $force_object = false, $flags = 0), `apiCsvResponse`($data, $delim = ",", $add_bom = false), `apiCsvFileResponse`($data, $filename, $delim = ",", $add_bom = true) | TBD | Helper methods for returning API responses from the `redcap_module_api` hook. See [Module API](../api.md) for more details.
