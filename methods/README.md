...
`deleteUserQuery`($key [, $pid]) | TBD | Deletes the user query with the specified key that was saved with the `saveUserQuery()` method. If a project ID is provided, the query is deleted for that project only. Requires **Access to all projects and data with maximum user privileges** permission. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
...
`getEdocPath`($edoc_id) | 16.0.5                       | Returns the full (safe) path to an edoc.
...
`getUser`([$username]) | 8.11.9                       | Returns a `User` object for the given username, or the current user if no username is specified.  This `User` object is [documented below](#user-object).  If the specified or current user cannot be found, an exception is thrown to encourage explicit handling of that scenario.  NOAUTH scenarios can be handled as follows:<br><code>if($module->isAuthenticated()){<br>&nbsp;&nbsp;&nbsp;&nbsp;$user = $module->getUser();<br>}</code>
`getUserQuery`($key [, $pid]) | TBD | Returns the user query with the specified key that was saved with the `saveUserQuery()` method. If a project ID is provided, the query for that project is returned. The resulting query can be executed with the `runUserQuery()` method. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
`getUserQueryKeys`([$pid]) | TBD | Returns an array of user query keys for the specified project ID. If no project ID is provided, system query keys are returned. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
...
`initializeTwig`([$whatever [, $options]]) | 14.6.4                       | Loads Twig classes and configures a Twig `Environment` for your module.  Support for the `options` parameter was added in 17.0.1.  See details on the [Twig page](../twig.md).
...
`runAdminQuery`($sql [, $parameters]) | TBD | Executes the provided query against the database using the provided optional parameters. Only `select` queries are allowed. This method can be used with any `select` query, but only administrators with the **Access to all projects and data with maximum user privileges** *AND* the **Install, upgrade, and configure External Modules** permissions.
`runUserQuery`($sql [, $parameters]) | TBD | Executes the provided query against the database using the provided optional parameters. Only `select` queries are allowed. This method can only be used with queries returned by the `getUserQuery()` method. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
...
`saveUserQuery`($key, $sql [, $pid]) | TBD | Saves a user query with the specified key and SQL statement. If a project ID is provided, the query is saved for that project only. The query can be retrieved by the `getUserQuery()` method and then executed by the `runUserQuery()` method. Requires **Access to all projects and data with maximum user privileges** permission. The *enable-user-queries* flag must be set to true in config.json to enable this method. See details on the [User-defined SQL Queries page](../user-queries.md).
...
`getField`($fieldName) | 10.8.2 | Returns a `Field` object as [documented below](#field-object) for the given field name.
`getForm`($formName) | 10.8.2 | Returns a `Form` object as [documented below](#form-object) for the given form name.
