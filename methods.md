...
getProjectsWithModuleEnabled([$includeCompleted]) | 8.11.6 | Returns an array of project ids for which the current module is enabled.  This especially useful for iterating over projects in cron jobs.  Completed projects are not included by default, but will be if `true` is passed as the optional first argument (added in TBD).
