...
disableModule($pid, $prefix = null) | TBD | Disables a module on a specified project. If the optional prefix parameter is not specified, the current module will be disabled.
...
getProjectsWithModuleEnabled([$includeCompleted, $statuses]) | 8.11.6 | Returns an array of project ids on which the current module is enabled.  This especially useful for iterating over projects in cron jobs.  By default, projects that are completed or in analysis/cleanup status are excluded.  Since REDCap version TBD, completed projects may be included by passing `true` as the optional first argument, and project statuses included may be overridden by passing an array of integers (0, 1, and/or 2) as the optional second argument.
...
isSuperUser() | TBD | Returns true if the current user is a super user (a.k.a. the "Access to all projects and data with maximum user privileges" permission).  The "View project as user" impersonation feature is taken into account.
