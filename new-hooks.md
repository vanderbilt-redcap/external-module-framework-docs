...
1. Add the hook to the REDCap source on your localhost using a code block like the following:
    ...
    ```
		- Please, do not pass an associative array (i.e. named keys) to `callHook`; while more readable, this requires EM developers implementing your hook to define their function with parameter names that **exactly** match the keys of your array.
